...
def train_collate_fn(data):
    """
    Flattens patch lists and strips out heterogeneous metadata keys.
    Used ONLY for training, as training doesn't require spatial metadata.
    """
    # 1. Flatten the list of lists (generated by RandCropByPosNegLabeld)
    if isinstance(data[0], (list, tuple)):
        data = [item for sublist in data for item in sublist]
    
    # 2. Strip everything except 'image' and 'label' to ensure uniform tensor collation
    clean_data = [{"image": item["image"], "label": item["label"]} for item in data]
